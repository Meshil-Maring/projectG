...
export default function VideoStories() {
  ...
  const seekBy = (deltaPct: number) => {
    const video = videoRef.current;
    if (!video?.duration) return;
    const pct = Math.min(Math.max(progress / 100 + deltaPct, 0), 1);
          ...
          <div className="flex-1 w-full">
            ...
            <div className="flex flex-col items-center gap-3 mt-4">
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2 bg-primary text-white rounded-full text-sm font-semibold hover:bg-primary-dark transition shadow-sm"
              >
                <LayoutGrid size={15} />
                See All Videos
              </button>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <motion.button
                ...
                onClick={() => setIsPlaying((p) => !p)}
                className="flex items-center gap-3 px-6 py-3 bg-primary text-white rounded-full font-semibold hover:bg-primary-dark transition shadow-md"
              >
